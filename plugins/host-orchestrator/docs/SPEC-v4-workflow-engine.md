...
- **Contrato del hook (lugar canónico; 4.12.0):** `scripts/wave-validate.sh --json` emite
  ...
  `tests.failing_test_files` son paths relativos a la raíz del repo; el hook puede sumar campos (`failed_names`, …) y el motor los ignora. Si trae `tests.failed` entero y `tests.failing_test_files` lista, **el validator los copia y no corre ninguna suite propia** (`tests_fuente: "hook"`). Si no los trae, corre la suite él (`tests_fuente: "suite"`). Sin hook → el validator autodetecta el stack (default JS: package manager + typecheck + test; `metrics.typecheck_errors`) y reporta el mismo contrato en su schema de salida.
