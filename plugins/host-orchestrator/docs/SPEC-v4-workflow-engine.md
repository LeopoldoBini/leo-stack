...
PASA ⇔  tests_propios == verde                      (los tests del slice, todos)
     ...
     ∧  lockfiles_ajenos ⊆ baseline.lockfiles_ajenos  (ningún package manager nuevo junto a otro)
```
...
- **Tests del diff con hook (4.12.1):** con `tests_fuente: "hook"` el validator solo lista los archivos de test del diff (`tests_del_diff`) y no los corre. El motor deriva en JS `tests_del_diff_verdes` = hay al menos uno y ninguno está en `failing_test_files`. Vale solo porque el hook corrió la suite entera (contrato de abajo). Sin hook, el validator los corre como antes. Origen: autopsia `cn-radar-al-dia-0921`, donde el paso del diff volvía a correr bats pesados que el hook ya había medido.
- **Baseline:** se captura UNA vez por wave, sobre `prd/X` recién refrescada (Fase 1), corriendo el mismo validador. `{metrics: {<nombre>: N, ...}, tests_failed: M, failing_test_files: [paths], lockfiles_ajenos: [paths]}`.
- **Contrato del hook (lugar canónico; 4.12.0):** `scripts/wave-validate.sh --json` emite
  `{"status": "ok" | "error" | "measurement_failed", "metrics": {"ts_errors": N, ...}, "tests": {"failed": M, "failing_test_files": ["test/x.bats", ...]}}`.
  `tests.failing_test_files` son paths relativos a la raíz del repo; el hook puede sumar campos (`failed_names`, …) y el motor los ignora. Si trae `tests.failed` entero y `tests.failing_test_files` lista, **el validator los copia y no corre ninguna suite propia** (`tests_fuente: "hook"`). Si no los trae, corre la suite él (`tests_fuente: "suite"`). **El hook tiene que correr la suite entera**: el motor usa su lista de rojos como verdad sobre todos los tests del repo. Un hook que corre un subconjunto rompe el atajo de abajo y no debe reportar `tests.failing_test_files`. Sin hook → el validator autodetecta el stack (default JS: package manager + typecheck + test; `metrics.typecheck_errors`) y reporta el mismo contrato en su schema de salida.
- **Cada comando del validator lleva su `cd <worktree> &&` (4.12.0).** El cwd de Bash se resetea entre llamadas. Origen, autopsia `cn-radar-al-dia-0921` (2026-09-21): el hook dio verde en el worktree del PR, y el paso "corré la suite de tests del repo" hizo que el validator improvisara y corriera `bats` desde la raíz del repo principal. Midió un rojo que no existía y pisó el verde: 2 bloqueos falsos, ~2 h de camino crítico. 17 de 37 validadores de esa corrida re-corrieron tests pesados por su cuenta.
- **Caché del gate por árbol de git (4.12.0):** el validator invoca el hook a través de `scripts/gate-cache.sh <hook> --json` (path absoluto en `args.gateCache`). Clave: `HEAD^{tree}` + argumentos. Guarda solo con el worktree limpio (`git status --porcelain` vacío) y la salida en `status: "ok"`, en `$(git rev-parse --git-common-dir)/host-orchestrator/gate-cache/`, compartida entre worktrees. TTL 6 h (`HO_GATE_CACHE_TTL`, en segundos); `HO_GATE_CACHE=off` la apaga. Hit/miss a stderr. Motivo: en la misma autopsia el mismo árbol se midió hasta 3 veces (gate de la issue, gate del PR sin cambios, baseline de la wave siguiente). Test: `scripts/test-gate-cache.sh`.
- **Lockfiles ajenos (4.12.0):** en toda medición el validator transporta `git ls-files` de los lockfiles (`package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `bun.lockb`, `bun.lock`) y el motor calcula en JS `lockfiles_ajenos`: los de carpetas donde conviven lockfiles de managers distintos. El gate falla si aparece uno que no estaba en el baseline. Origen: el applier del review de `cn-radar-al-dia-0921` corrió `npm install` en un repo pnpm y coló dos `package-lock.json`. El JS del workflow no tiene harness de tests; `lockfilesAjenos` y `gate` se verificaron extrayéndolas a mano al publicar 4.12.0.
...
**Umbral del juez (4.12.0, autopsia `cn-radar-al-dia-0921`):** APLICAR solo cuatro clases — bug real, seguridad, requisito del spec incumplido, contrato roto. Estilo, refactor, nombres, docs y redundancias van a RECHAZAR con razón `seguimiento: …`, y el PR final las lista en «Sugerencias de seguimiento» (también salen en `seguimientos` del reporte). Regla dura: un fix que cambia la forma de una respuesta, payload o tipo compartido entre capas (sacar o renombrar un campo, cambiar un tipo) va a HUMANO. En esa corrida el juez aprobó sacar un `id` «redundante» de una respuesta de API; el frontend tenía su propia copia de los tipos, `tsc` no lo vio y la pantalla se rompió. Esa corrida aprobó 64 fixes para un tema chico.

**Package manager (4.12.0):** implementer, applier y serializers que instalan usan el manager del lockfile existente de cada carpeta y solo crean o tocan ese lockfile (`REGLA_DEPS` en el motor). El applier trabaja con las dependencias instaladas y reinstala solo si el fix es de dependencias. El gate lo respalda con `lockfiles_ajenos` (§3.3).
...
- **Deps y spec padre por grafo nativo, sin fallback textual.** `issue_dependencies_summary.blocked_by` cuenta solo bloqueantes ABIERTOS: es el gate del scout como entero de GitHub en vez de un LLM leyendo prosa. `parent_issue_url` localiza el spec. Trampa de herramienta: **`gh issue list --json` no expone ninguno de los dos** — hay que ir por `gh api`. Los repos heredados fallan ruidoso en precondiciones (§3.13b). **Bloqueante saldado en la integradora (4.5.0):** un bloqueante del scope con PR mergeado a la rama integradora se descuenta del gate aunque GitHub lo cuente abierto — su código ya está en la base desde la que branchea el dependiente, y la issue recién cierra con el PR final a la default branch. Sin el descuento, una cadena A←B←C avanzaba un eslabón por corrida (leo-stack #26). Bloqueantes fuera del scope siguen rigiéndose por el grafo tal cual.
